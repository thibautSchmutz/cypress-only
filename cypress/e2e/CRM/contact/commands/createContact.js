module.export = Cypress.Commands.add(
    ...
    ({
      ...
      cy.getByDataBot("contact-overview__form--firstname").as("contact_firstname")
    ...
    cy.log(dateOfBirth)
      ...
      cy.getByDataBot("contact-overview__tablist-social-networks").as("contact_social_networks");
      cy.get("@contact_social_networks").then((socialNetworksContainer) => {
          cy.wrap(socialNetworksContainer).find('div[role=button]').click({ force: true })
        })
        
      cy.getByDataBot("contact-overview__tablist-social-networks--facebook").as("contact_facebook");
      cy.get("@contact_facebook").type(facebook)

      cy.getByDataBot("contact-overview__tablist-social-networks--twitter").as("contact_twitter");
      cy.get("@contact_twitter").type(twitter)

      cy.getByDataBot("contact-overview__tablist-social-networks--linkedin").as("contact_linkedin");
      cy.get("@contact_linkedin").type(linkedin)


      cy.getByDataBot("contact-overview__btn-bottom--save").as("contact_submit_btn")
